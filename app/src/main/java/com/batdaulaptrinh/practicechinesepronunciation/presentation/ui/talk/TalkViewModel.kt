...
@HiltViewModel
class TalkViewModel @Inject constructor(private val getDisplayListUseCase: GetDisplayListUseCase) :
    ViewModel() {
    val speeches = MutableLiveData<List<NewSpeechEntity>>()
    fun loadSpeeches(lessonTitle: String) {
        viewModelScope.launch {
            speeches.postValue(getDisplayListUseCase.getListVocabUseCase(lessonTitle))
        }
    }
}